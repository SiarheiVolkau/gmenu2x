	  ...
	  cpufreq(0), hours(99), minutes(99), timeIs24(true), manualEnabled(false)
{
	...
	margin = gmenu2x.skinConfInt["bottomBarHeight"] / 4;

	updateBatteryIcon();

	auto sd = gmenu2x.sc.skinRes("imgs/sd.png");
	...
	auto cpu_icon = gmenu2x.sc.skinRes("imgs/cpu.png");
	...
	manual = gmenu2x.sc.skinRes("imgs/manual.png");
...
void BottomBar::showCpuFreq(unsigned long mhz)
{
	...
	if (!mhz && !!cpufreq) {
		...
		auto new_cpu_text = std::shared_ptr<OffscreenSurface>(text_s.release());
...
void BottomBar::updateDiskFreeText()
{
	...
	auto text_s = gmenu2x.font->render(ss.str());
	auto new_sd_text = std::shared_ptr<OffscreenSurface>(text_s.release());
...
void BottomBar::updateTimeText()
{
	...
	auto text_s = gmenu2x.font->render(ss.str());
	auto new_clock_text = std::shared_ptr<OffscreenSurface>(text_s.release());
...
void BottomBar::updateBatteryIcon()
{
	...
	if (battery_icon == icon)
		return;

	icon->setMargins(margin, 0, 0, 0);

	if (battery_icon) {
		addSibling(battery_icon, icon);
